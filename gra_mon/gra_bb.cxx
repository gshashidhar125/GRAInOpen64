...
// Remove the Complementary LRange from the list of Global LRanges passing through the GRA_BB.
// If 'lrange' is of type local, then the function has no effect.
void
GRA_BB::Remove_Global_LRange(LRANGE *lrange) {

	GRA_REGION *cregion = gra_region_mgr.Complement_Region();
	ISA_REGISTER_CLASS Register_Class = lrange->Rc();
	global_lranges[Register_Class] = global_lranges[Register_Class]->Remove_Neighbor(lrange, cregion->Subuniverse(Register_Class));
}
