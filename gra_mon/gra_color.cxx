  ...
  for (iter.Init(region,rc); ! iter.Done(); iter.Step()) {
    ...
    if ( lr->Has_Wired_Register() ) {
      ...
      if ( lr->Has_Preference() ) {
		...
		#ifdef REG_ALLOC_PROJ
		// The grouping of LRanges into 'ready' and 'not_ready' must not consider those Complementary LRanges that are to be Spilled.
		&& !lr->Spilled()
		#endif
		) {
...
#ifdef REG_ALLOC_PROJ
  #ifdef KEY // for key, leave ready queue empty; rank purely based on priority
  // If the Registers available is less than the neighbors of the Lranges, those LRanges are candidates for Spilling and are put into 'not_ready' list.
...
static 
#ifdef REG_ALLOC_PROJ
// Simplify function will return the candidate Complementary LRange to be spilled which had many neighbors than the Register Count. If there are 
// no LRanges to be Spilled then the function returns NULL.
LRANGE *
#else
void
#endif
...
/////////////////////////////////////
{
  ...
  #ifdef REG_ALLOC_PROJ
  // To print the 'ready' and 'not_ready' list of LRanges.
  /*fprintf(GRA_dump_file, "\n\nReady Lranges:\n");
  for (int i = 1; i < LRPRQ_Size(&ready); i++) {
  		LRANGE *temp = LRPRQ_Ith(&ready, i);
  		fprintf(GRA_dump_file, "\tLRange : %d, Type = %s\n", temp->Id(), (temp->Type() == 2)?"Local":"Complement" );
  }

  fprintf(GRA_dump_file, "\n\nNot_Ready Lranges:\n");
  for (int i = 1; i < LRPRQ_Size(&not_ready); i++) {
  		LRANGE *temp = LRPRQ_Ith(&not_ready, i);
  		fprintf(GRA_dump_file, "\tLRange : %d, Type = %s\n", temp->Id(), (temp->Type() == 2)?"Local":"Complement" );
  }*/
  #endif
#ifdef KEY // for key, leave ready queue empty; rank purely based on priority
  for (;;) {
    if ( LRPRQ_Size(&ready) != 0 ) {
	  ...
	  #ifdef REG_ALLOC_PROJ
	  // The 'not_ready' list contains the LRanges that need to be spilled. We only consider the Complementary LRanges for Spilling here. Local LRanges are 
	  // handled by Local_Register_Allocator. Complementary LRange which has least Priority is selected for Spilling.
	    lr = NULL;
  		fprintf(GRA_dump_file, "\n\tComparing LRanges to Spill:\n");
 	    for (int i = 1; i <= LRPRQ_Size(&not_ready); i++) {
  		  LRANGE *temp = LRPRQ_Ith(&not_ready, i);
  		  fprintf(GRA_dump_file, "\t\t %d - LRange : %d, Type = %s, Priority = %f\n", i, temp->Id(), (temp->Type() == 2)?"Local":"Complement" , temp->Priority());

		  if (lr == NULL && temp->Type() == LRANGE_TYPE_COMPLEMENT)
		  	lr = temp;
		  else if (lr != NULL && temp->Type() == LRANGE_TYPE_COMPLEMENT && lr->Priority() > temp->Priority())
		  	lr = temp;
     	}

		if (lr == NULL)
			lr = LRPRQ_Ith(&not_ready, 1);

	    if (lr->Type() == LRANGE_TYPE_LOCAL) {
	  
			LRPRQ_Remove(&not_ready, lr);
	        cl->Push(lr);
    	    GRA_Trace_Color_LRANGE("Listing.",lr);
	        lr->Listed_Set();
	    }
...
#ifdef REG_ALLOC_PROJ
// For the Complementary LRange 'spill_lrange' to be spilled, we need to Update the interference graph by removing the 'spill_lrange' from its neighbors
// interference nodes.
void Update_Interference_Graph(LRANGE *spill_lrange) {

	LRANGE_LIVE_GBB_ITER live_gbb_iter;
	GRA_REGION_RC_LRANGE_ITER lrange_iter;
	LRANGE_NEIGHBOR_ITER neighbor_iter;
	GRA_REGION *cregion = gra_region_mgr.Complement_Region();
	ISA_REGISTER_CLASS Register_Class = spill_lrange->Rc();

	GRA_Note_Spill(spill_lrange);

	// The 'spill_lrange' will remove every gbb from its 'live_bb_set' where the Complementary LRange was live.
	// This makes every Local LRange that are live in those 'live_bb_set' remove the 'spill_lrange' from its interference node.
	// The interference between Complementary and Local are through these 'live_bb_set'.
	for (live_gbb_iter.Init(spill_lrange); ! live_gbb_iter.Done(); live_gbb_iter.Step()) {
		GRA_BB* live_gbb = live_gbb_iter.Current();
		live_gbb->Remove_Global_LRange(spill_lrange);
		spill_lrange->Remove_Live_BB(live_gbb);
	}

	// Remove other Complementary Interfering LRanges. These are indicated in 'u.c.neighbors' INTERFERE node of every LRANGE.
	for (neighbor_iter.Init(spill_lrange, cregion); !neighbor_iter.Done(); neighbor_iter.Step()) {
		LRANGE *neighbor = neighbor_iter.Current();
		
		neighbor->Remove_Neighbor(spill_lrange, cregion);

		if (neighbor->Type() == LRANGE_TYPE_LOCAL) {

			printf("\nNeighbor of the LRange to be spilled is a Local Lrange\n");
		}
	}

	for (lrange_iter.Init(cregion, Register_Class); !lrange_iter.Done(); lrange_iter.Step()) {

		LRANGE *lrange = lrange_iter.Current();
		if (lrange != spill_lrange) {
			if (!lrange->Allocated()) {

				if (lrange->Listed()) 
					lrange->Not_Listed_Set();		
			}
			else 
				printf("\nThe LRange to be made Not_listed is already Allocated\n");
		}
	}

    fprintf(GRA_dump_file, "\n\n\t Spilling LRange = %d, Type = %s, Neighbor_count = %d\n\n ", spill_lrange->Id(), (spill_lrange->Type() == 2) ?"Local":"Complement", spill_lrange->Neighbor_Count());
}
#endif


//#ifdef REG_ALLOC_PROJ
// Dump data about LRanges in the REGISTER_CLASS rc
void Dump_Data(ISA_REGISTER_CLASS  rc) {
	...
	for (lrange_iter.Init(region, rc); !lrange_iter.Done(); lrange_iter.Step()) {
		...
		LRANGE *lrange = lrange_iter.Current();
		fprintf(GRA_dump_file, "\t\t LRange = %d Type = %s Neighbor_count = %d Flags = ", lrange->Id(), (lrange->Type() == 2) ?"Local":"Complement", lrange->Neighbor_Count());
		  
		if (lrange->Listed())
			fprintf(GRA_dump_file, "Listed, ");
		if (lrange->Allocated())
			fprintf(GRA_dump_file, "Register Allocated, ");
		if (lrange->Has_Wired_Register())
			fprintf(GRA_dump_file, "Wired, ");
		if (lrange->Spilled())
			fprintf(GRA_dump_file, "Spilled ");

		if (!lrange->Type() == LRANGE_TYPE_COMPLEMENT) {
		  
			fprintf(GRA_dump_file, "BB = %d", lrange->Gbb()->Bb()->id);
		}
...
/////////////////////////////////////
{
  ...
  char buff[100];
  #ifdef REG_ALLOC_PROJ
    BOOL loop_again;	// A Boolean value to indicate whether the simplify phase has to be run once again.
	int No_Of_Turns;	// The Number of times the Simplify phase has run for a particular Interference Graph corresponding to ISA_REGISTER_CLASS.
  #endif

  //#ifdef REG_ALLOC_PROJ
	static int Region_Number = 1;	// Explicitly given ID for this Complement region which is undergoing Coloring process.
	fprintf(GRA_dump_file, "Region = %d\n", Region_Number);
  //#endif
  priority_count = 0.0;
  ...
  FOR_ALL_ISA_REGISTER_CLASS_IN_REVERSE( rc ) {
	...
	#ifdef REG_ALLOC_PROJ
	// Do the Simplification until there is no Complementary LRanges whose neighbors are more in number than the Registers available. After each Simplification,
	// if a Complementary Lrange is to be Spilled then the neighbors list for that Complementary Lrange has to Updated. This is done in the Update_Interference_
	// Graph.
      do {

        fprintf(GRA_dump_file, "\tSimplification Turn %d:\n", ++No_Of_Turns);
		Dump_Data(rc);
        if ((spill_lrange = Simplify(region,rc,&cl)) != NULL) {
		  Update_Interference_Graph(spill_lrange);
		  loop_again = TRUE;
	    }
		else
		  loop_again = FALSE;
